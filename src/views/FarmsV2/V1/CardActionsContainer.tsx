...
const CardActions: React.FC<FarmCardActionsProps> = ({ farm, account }) => {
  ...
  const { pid, type } = useFarmFromSymbol(farm.lpSymbol) // ant touch const { pid, lpAddresses, type } = useFarmFromSymbol(farm.lpSymbol)
  const { allowance, stakedBalance } = useFarmUser(pid, type)
  // const web3 = useWeb3()
