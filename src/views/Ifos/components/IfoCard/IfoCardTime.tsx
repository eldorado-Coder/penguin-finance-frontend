...
import { Link, Text } from '@penguinfinance/uikit'
...
const IfoCardTime: React.FC<IfoCardTimeProps> = ({ isLoading, status, secondsUntilStart, secondsUntilEnd, block }) => {
  ...
  return (
    <Details>
      ...
      <Link href={`https://cchain.explorer.avax.network.com/block/countdown/${block}`} target="blank" rel="noopener noreferrer" ml="8px">
        (blocks)
      </Link>
    </Details>
  )
