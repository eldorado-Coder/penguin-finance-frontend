...
export const fetchMasterChefGlobalData = async () => {
  const [lydPerSec] = await multicall(getFarmMasterChefAbi('Lydia'), [
    {
      address: getFarmMasterChefAddress('Lydia'),
      name: 'lydPerSec',
    },
  ])

  return { lydPerSec: new BigNumber(lydPerSec).div(new BigNumber(10).pow(18)).toNumber() }
}
